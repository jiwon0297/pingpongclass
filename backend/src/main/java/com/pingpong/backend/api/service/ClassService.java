...
public class ClassService {
    ...
    public void save(final ClassRequest req) {
        ...
        List<ClassStudentEntity> classStudentEntityList = req.getClassStudentEntityList();
        for(ClassStudentEntity entity: classStudentEntityList){
            ClassStudentEntity classStudentEntity = classStudentRepository.save(entity);
        }
    }
    ...
    //수업 수정
    public void modify(ClassRequest req){
        TimetableEntity timetableEntity = timetableRepository.getOne(req.getTimetableId());
        ClassEntity classEntity = classRepository.getOne(req.getClassId());
        classEntity.update(timetableEntity, req.getClassTitle(), req.getClassDesc(), req.getClassDay());
        //수정사항도 자동으로 반영해주면 안되는건가....제발요.....
    }
